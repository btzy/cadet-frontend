    ...
    result:
      actionType === actionTypes.DEBUG_RESUME
        ...
        ? call(runInContext, code, context, {
          ...
          : call(wasm_compile_and_run, code),
    /**
...
async function wasm_compile_and_run(code: string): Promise<Sourceror.Result> {
  return Sourceror.compile(code, {}).then((wasmModule: WebAssembly.Module) =>
    Sourceror.run(wasmModule)
  );
}
